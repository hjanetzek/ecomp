...
static Bool
ensureLargerClipCapacity(PolygonSet *pset)
{
...
static void
freeClipsPolygons(PolygonSet *pset)
{
   int k;
...
static Bool
processIntersectingPolygons(CompScreen *s, PolygonSet *pset)
{
   int j;
             ...
             for (k = 0; k < p->nSides; k++)
               {
                  ...
                  GLfloat tx;
                  GLfloat ty;
                  if (c->texMatrix.xy != 0.0f || c->texMatrix.yx != 0.0f) // "non-rect" coordinates
                    {
...
void
polygonsDrawCustomGeometry(CompScreen *s, CompWindow *w)
{
   ANIM_WINDOW(w);
...
void
polygonsPrePaintWindow(CompScreen *s, CompWindow *w)
{
   ANIM_WINDOW(w);
   if (aw->polygonSet)
...
void
polygonsPostPaintWindow(CompScreen *s, CompWindow *w)
{
   ANIM_WINDOW(w);
