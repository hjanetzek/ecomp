			  ...
			  VideoSource *source)
{
    ...
    */
    x1 = MAX (0, vw->x);
    y1 = MAX (0, vw->y);
    x2 = MIN (w->width, vw->width);
    y2 = MIN (w->height, vw->height);
    //printf("updateWindowVideoContext %d:%d %dx%d\n", x1,y1, x2,y2);
    
    vw->context->width  = vw->width; //x2 - x1;
    vw->context->height = vw->height; //y2 - y1;
    ...
    vw->context->box.extents.x2 = x2 + x1;
    vw->context->box.extents.y2 = y2 + y1;
    /*    printf("updateWindowVideoContext %d:%d %dx%d\n", vw->context->box.extents.x1,
	   vw->context->box.extents.y1, 
	   vw->context->box.extents.x2,
	   vw->context->box.extents.y2);*/
...
videoWindowUpdate (CompWindow *w)
{
    ...
    result = XGetWindowProperty (w->screen->display->display, vw->videoWindow,
		  ...
		  XEvent      *event)
{
    ...
    case ClientMessage:
      ...
      if (event->xclient.message_type == vd->videoAtom)
      {
	...
	if (w)
	  {
	    ...
	    vw->videoWindow = event->xclient.window;
	    vw->damage = XDamageCreate (d->display, 
					//(Pixmap)event->xclient.data.l[1],
	...
	{
	printf("got video property\n");
	  
	    w = findWindow (d, event->xproperty.window);
	    if (w)
		videoWindowUpdate (w);
	}
	break;
		...
		for (w = s->windows; w; w = w->next)
		  {
			...
			if(vw->videoWindow == de->drawable) 
			  {
			    /*  printf("got video damagen\n %d:%d %dx%d\n %d:%d %dx%d\n", 
				   de->geometry.x, de->geometry.y, de->geometry.width, de->geometry.height, 
				   de->area.x, de->area.y, de->area.width, de->area.height);
			    */
			    vw->x = de->geometry.x - w->attrib.x;
			    vw->y = de->geometry.y - w->attrib.y;
			    vw->width = de->geometry.width;
			    vw->height = de->geometry.height;
			    
			    box.x1 = de->geometry.x;
			    box.y1 = de->geometry.y;
			    box.x2 = de->geometry.width;
			    box.y2 = de->geometry.height;
