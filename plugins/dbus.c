...
#define ECOMP_DBUS_SERVICE_NAME	            "org.freedesktop.ecomp"
#define ECOMP_DBUS_INTERFACE			    "org.freedesktop.ecomp"
#define ECOMP_DBUS_ROOT_PATH			    "/org/freedesktop/ecomp"

#define ECOMP_DBUS_ACTIVATE_MEMBER_NAME            "activate"
#define ECOMP_DBUS_DEACTIVATE_MEMBER_NAME          "deactivate"
#define ECOMP_DBUS_SET_MEMBER_NAME                 "set"
#define ECOMP_DBUS_GET_MEMBER_NAME                 "get"
#define ECOMP_DBUS_GET_METADATA_MEMBER_NAME	    "getMetadata"
#define ECOMP_DBUS_LIST_MEMBER_NAME		    "list"
#define ECOMP_DBUS_GET_PLUGINS_MEMBER_NAME	    "getPlugins"
#define ECOMP_DBUS_GET_PLUGIN_METADATA_MEMBER_NAME "getPluginMetadata"

#define ECOMP_DBUS_CHANGED_SIGNAL_NAME		    "changed"
#define ECOMP_DBUS_PLUGINS_CHANGED_SIGNAL_NAME	    "pluginsChanged"
...
dbusIntrospectStartInterface (xmlTextWriterPtr writer)
{
    ...
    xmlTextWriterWriteAttribute (writer, BAD_CAST "name",
				 BAD_CAST ECOMP_DBUS_SERVICE_NAME);
}
				 ...
				 CompDisplay	*d)
{
    ...
    dbusIntrospectAddMethod (writer, ECOMP_DBUS_GET_PLUGINS_MEMBER_NAME, 1,
			     "as", "out");
    dbusIntrospectAddMethod (writer,
			     ECOMP_DBUS_GET_PLUGIN_METADATA_MEMBER_NAME, 7,
			     "s", "in", "s", "out", "s", "out", "s", "out",
			     "b", "out", "as", "out", "as", "out");
    dbusIntrospectAddSignal (writer,
			     ECOMP_DBUS_PLUGINS_CHANGED_SIGNAL_NAME, 0);
				   ...
				   char           **path)
{
    ...
    dbusIntrospectAddMethod (writer, ECOMP_DBUS_LIST_MEMBER_NAME, 1,
			     "as", "out");
				   ...
				   char           **path)
{
	...
	if (strcmp (option->name, path[2]) == 0)
	{
	    ...
	    {
		dbusIntrospectAddMethod (writer,
					 ECOMP_DBUS_GET_MEMBER_NAME, 1,
					 type, "out");
		dbusIntrospectAddMethod (writer,
					 ECOMP_DBUS_SET_MEMBER_NAME, 1,
					 type, "in");
		dbusIntrospectAddSignal (writer,
					 ECOMP_DBUS_CHANGED_SIGNAL_NAME, 1,
					 type, "out");
	    }
 ...
 *
 * dbus-send --type=method_call --dest=org.freedesktop.ecomp \
 * /org/freedesktop/ecomp/rotate/allscreens/rotate_to	      \
 * org.freedesktop.ecomp.activate			      \
 * string:'root'					      \
 ...
 * dbus-send --type=method_call --dest=org.freedesktop.ecomp \
 * /org/freedesktop/ecomp/cube/allscreens/unfold	      \
 * org.freedesktop.ecomp.activate			      \
 * string:'root'					      \
 * int32:`xwininfo -root | grep id: | awk '{ print $4 }'`
 *
 * dbus-send --type=method_call --dest=org.freedesktop.ecomp \
 * /org/freedesktop/ecomp/cube/allscreens/unfold	      \
 * org.freedesktop.ecomp.deactivate			      \
 * string:'root'					      \
 * int32:`xwininfo -root | grep id: | awk '{ print $4 }'`
 *
 */
 ...
 *
 * dbus-send --type=method_call --dest=org.freedesktop.ecomp \
 * /org/freedesktop/ecomp/core/allscreens/command0	      \
 * org.freedesktop.ecomp.set				      \
 * string:'firefox'
 *
 ...
 *
 * dbus-send --type=method_call --dest=org.freedesktop.ecomp \
 * /org/freedesktop/ecomp/core/allscreens/active_plugins     \
 * org.freedesktop.ecomp.set				      \
 ...
 *
 * dbus-send --type=method_call --dest=org.freedesktop.ecomp \
 * /org/freedesktop/ecomp/core/allscreens/run_command0	      \
 * org.freedesktop.ecomp.set				      \
 ...
 * --dest=org.freedesktop.ecomp		    \
 * /org/freedesktop/ecomp/core/allscreens/command0 \
 * org.freedesktop.ecomp.get
 */
static Bool
 ...
 * dbus-send --print-reply --type=method_call \
 * --dest=org.freedesktop.ecomp	      \
 * /org/freedesktop/ecomp/core/allscreens    \
 * org.freedesktop.ecomp.list
 */
static Bool
 ...
 * --dest=org.freedesktop.ecomp			\
 * /org/freedesktop/ecomp/core/allscreens/run_command0 \
 * org.freedesktop.ecomp.getMetadata
 */
static Bool
 ...
 * dbus-send --print-reply --type=method_call \
 * --dest=org.freedesktop.ecomp	      \
 * /org/freedesktop/ecomp		      \
 * org.freedesktop.ecomp.getPlugins
 */
static Bool
 ...
 * --dest=org.freedesktop.ecomp	      \
 * /org/freedesktop/ecomp		      \
 * org.freedesktop.ecomp.getPluginMetadata   \
 * string:'png'
 */
static Bool
		   ...
		   void           *userData)
{
					 ...
					 "Introspect"))
	{
		...
		return DBUS_HANDLER_RESULT_HANDLED;
				 ...
				 ECOMP_DBUS_GET_PLUGIN_METADATA_MEMBER_NAME))
	{
			       ...
			       char	       *path)
{
    ...
    signal = dbus_message_new_signal (path,
				      ECOMP_DBUS_SERVICE_NAME,
				      ECOMP_DBUS_CHANGED_SIGNAL_NAME);
				      ...
				      char	  *plugin)
{
    ...
    if (o)
    {
	sprintf (path, "%s/%s/allscreens/%s", ECOMP_DBUS_ROOT_PATH,
		 plugin, o->name);
				     ...
				     char	*plugin)
{
    ...
    if (o)
    {
	sprintf (path, "%s/%s/screens%d/%s", ECOMP_DBUS_ROOT_PATH,
			      ...
			      char           *pluginName)
{
    ...
    snprintf (objectPath, 256, "%s/%s", ECOMP_DBUS_ROOT_PATH, pluginName);
    dbus_connection_register_object_path (connection, objectPath,
					  &dbusMessagesVTable, d);

    /* register plugin/screen path */
    snprintf (objectPath, 256, "%s/%s/%s", ECOMP_DBUS_ROOT_PATH,
			     ...
			     char           *pluginName)
{
    ...
    /* register plugin/screen path */
    snprintf (objectPath, 256, "%s/%s/screen%d", ECOMP_DBUS_ROOT_PATH,
			       ...
			       CompDisplay    *d)
{
    ...
    while (nPlugins--)
    {
	snprintf (path, 256, "%s/%s/allscreens", ECOMP_DBUS_ROOT_PATH,
						 pl->value[nPlugins].s);
			      ...
			      CompScreen    *s)
{
    ...
    while (nPlugins--)
    {
	snprintf (path, 256, "%s/%s/screen%d", ECOMP_DBUS_ROOT_PATH,
						 pl->value[nPlugins].s,
						 s->screenNum);
			  ...
			  CompDisplay *d)
{
    ...
    if (status)
    {
	snprintf (objectPath, 256, "%s/%s/%s", ECOMP_DBUS_ROOT_PATH, p->vTable->name, "allscreens");
			 ...
			 CompScreen *s)
{
    ...
    if (status)
    {
	snprintf (objectPath, 256, "%s/%s/screen%d", ECOMP_DBUS_ROOT_PATH, p->vTable->name, s->screenNum);
			      ...
			      void	 *closure)
{
    ...
    signal = dbus_message_new_signal (ECOMP_DBUS_ROOT_PATH,
				      ECOMP_DBUS_SERVICE_NAME,
				      ECOMP_DBUS_PLUGINS_CHANGED_SIGNAL_NAME);
		 ...
		 CompDisplay *d)
{
    ...
    snprintf (objectPath, 256, "%s/core/allscreens", ECOMP_DBUS_ROOT_PATH);
		 ...
		 CompDisplay *d)
{
    ...
    dbus_bus_release_name (dd->connection, ECOMP_DBUS_SERVICE_NAME, NULL);
		...
		CompScreen *s)
{
    ...
    snprintf (objectPath, 256, "%s/%s/screen%d", ECOMP_DBUS_ROOT_PATH, "core", s->screenNum);
