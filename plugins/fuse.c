...
static struct fuse_lowlevel_ops ecomp_ll_oper = {
    .lookup  = ecomp_lookup,
    .getattr = ecomp_getattr,
    .setattr = ecomp_setattr,
    .readdir = ecomp_readdir,
    .open    = ecomp_open,
    .read    = ecomp_read,
    .write   = ecomp_write,
    .release = ecomp_release,
    .fsync   = ecomp_fsync
};
		 ...
		 CompDisplay *d)
{
    ...
    fd->session = fuse_lowlevel_new (NULL,
				     &ecomp_ll_oper, sizeof (ecomp_ll_oper),
				     (void *) d);
    if (!fd->session)
    {
